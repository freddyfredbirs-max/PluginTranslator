...
🛡️ Защита плейсхолдеров: {0}, %player%, [bracket], <tag>, &a/§a цветовые коды, MiniMessage-теги, экранированные символы (\:, \!, '') и т.д.
...
Парсинг — под каждый формат свой обработчик (YamlHandler, JsonHandler, PropertiesHandler), учитывающий особенности синтаксиса (кавычки в YAML, экранирование в .properties и т.д.)
Маскировка — единый набор regex-паттернов защищает технические токены перед отправкой на перевод
Скан — отдельный проход ищет token-подобные конструкции, которые НЕ попали под защиту, и показывает их до перевода
Перевод — через Google Translate, с кэшированием и автоповтором при сбоях
Сохранение — переведённый файл сохраняется рядом с оригиналом в том же формате


Установка и запуск

Вариант 1 — готовый .exe (Windows, без Python):
Скачай PluginTranslator.exe из раздела Releases, запусти, выбери файл локализации через интерфейс.

Вариант 2 — из исходников:

bashpip install ruamel.yaml   # или дай приложению установить самому при первом запуске
...
🛡️ Placeholder protection: {0}, %player%, [bracket], <tag>, &a/§a color codes, MiniMessage tags, escaped characters (\:, \!, ''), and more
...
Parsing — a dedicated handler per format (YamlHandler, JsonHandler, PropertiesHandler) accounts for that format's quirks (YAML quoting, .properties escaping, etc.)
Masking — a shared set of regex patterns protects technical tokens before they're sent for translation
Scan — a separate pass surfaces token-like constructs NOT caught by the protection patterns, before translation runs
Translation — via Google Translate, with caching and automatic retry on failures
Saving — the translated file is saved next to the original, in the same format


Install & run

Option 1 — prebuilt .exe (Windows, no Python needed):
Download PluginTranslator.exe from Releases, run it, pick your locale file through the interface.

Option 2 — from source:

bashpip install ruamel.yaml   # or let the app install it automatically on first run
python PluginTranslator.py
